...
| XA-3 | 責務担当 | S3 | 主張された能力ごとに、入力から出力までの処理経路を構成要素の列として列挙し、各要素を特定する。担当が特定できない処理段が1つでもあれば finding | 全能力の全処理段に担当構成要素が存在する |
...
検出性能(recall / precision)の測定手順は evals/seeding.md に定義する。既知の虚偽を型ごとに注入した複製リポジトリへのブラインド監査を機械採点する。テストケースと assertion は evals/evals.json。スキル改訂時はこのベンチマークで前バージョンと比較する。
